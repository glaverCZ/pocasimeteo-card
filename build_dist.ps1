...
# Find the insertion point (right after the opening IIFE and version log)
# Look for: (() => {\n  console.log(...)\n\n  // Detekuj...
$pattern = "(?s)\(\(\) => \{[^\{]+console\.log\([^\)]+\);[\r\n]+"
if ($jsContent -match $pattern) {
    # Create replacement with embedded icons
    $replacement = "(() => {`r`n  console.log('[PočasíMeteo Card] Version 2.2.3 loading...');`r`n`r`n  // Embedded Base64 icons (auto-generated by build_dist.ps1)`r`n" + $iconsObject
    ...
    Write-Host "Looking for pattern: (() => { followed by console.log"
    exit 1
}
