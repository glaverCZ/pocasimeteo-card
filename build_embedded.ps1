...
# Find the insertion point using regex
# Look for the opening of the IIFE and insert EMBEDDED_ICONS right after it
if ($jsContent -match '(?s)(\(\(\) => \{[\r\n]+)(  // Detekuj base path)') {
    $beforeIcons = $matches[1]
    $afterIcons = $jsContent.Substring($matches[0].Length - $matches[2].Length)

    Write-Host "Found insertion point"

    # Build new content with EMBEDDED_ICONS
    $newContent = $beforeIcons + "  // Embedded Base64 icons (auto-generated by build_embedded.ps1)`n" + $iconsObject + "`n" + $afterIcons
    $jsContent = $newContent
} else {
    Write-Host "ERROR: Could not find insertion point in JS file!"
    Write-Host "Looking for pattern: '(() => {' followed by '// Detekuj base path'"
    exit 1
}
